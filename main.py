...
if not TOKEN: raise ValueError("DISCORD_TOKEN není nastaven v prostředí!")
...
async def equation_command(interaction: discord.Interaction, equation: str):
    ...
    if len(equation) > 100: 
        text = '❌ Equation is too long'
    else:
        try:
            result = await asyncio.wait_for(
                asyncio.to_thread(simple_eval, equation), timeout=1.0)
            text = f"**Solve equation:** `{equation} = {result}`"
            if len(text) > 2000: 
                text = '❌ Response is too long'
        except asyncio.TimeoutError:
            text = "❌ Error: Calculation timed out (took longer than 1 second)."
